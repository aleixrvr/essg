---
title: "Impact Factors Radiology"
output:
...
for(outcome in outcomes){
  ...
  print(h3('Radiology'))
  dt <- dades[, .SD, .SDcols = c(outcome, confounders, surgery, reinterventions, radiology)]
  print(glue("Number of observations after removing missings: {na.omit(dt)[, .N]}"))
  create_model(dt, outcome) %>% 
    format_table(radiology) %>% 
    print
  
  print(h3('Resonance'))
  dt <- dades[, .SD, .SDcols = c(outcome, confounders, surgery, reinterventions, resonance)]
  print(glue("Number of observations after removing missings: {na.omit(dt)[, .N]}"))
  create_model(dt, outcome) %>% 
    format_table(resonance) %>% 
    print
  
  print(h3('Mental Component'))
  dt <- dades[, .SD, .SDcols = c(outcome, confounders, surgery, reinterventions, mental)]
  print(glue("Number of observations after removing missings: {na.omit(dt)[, .N]}"))
  create_model(dt, outcome) %>% 
    format_table(mental) %>% 
    print
