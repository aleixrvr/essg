...
```{r}
clinical_data %<>% 
  .[as.Date(`st1. Date of Stage 1`) < as.Date('2015-12-15')] 

reiqs_data <- clinical_data %>% 
  .[!is.na(`2 YEAR VISIT - Date of visit`) | !is.na(`3 YEAR VISIT - Date of visit`)]

patients_visited_5y <- clinical_data %>% 
  .[!is.na(`5 YEAR VISIT - Date of visit`) | !is.na(`6 YEAR VISIT - Date of visit`), 
    `Code of the patient` %>% uniqueN]

total_patients <- clinical_data[, `Code of the patient` %>% uniqueN]
patients_visited <- reiqs_data[, `Code of the patient` %>% uniqueN]
```
...
- Number of patients with visit in 5 years: `r patients_visited_5y`
...
for(var_ in vars$radiology ){
  ...
  for(var_y in vars_years){
    cat(var_y)
    cat('\n')
    res <- stats_fun(reiqs_data[[var_y]])
    for( res_name in names(res) ){
      print(res_name)
      print(res[[res_name]])
    }
