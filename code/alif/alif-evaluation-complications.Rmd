...
```{r ate_comps, warning=FALSE, echo=FALSE, comment="", include=FALSE}
outcomes <- complications[, `Category of the complication` %>% unique]
outcomes <- outcomes %!in% "Central neurologic complications"

# predictive_variates <- c(treatment_name, 'Propensity')
results_outcomes <- list()
is_classification <- TRUE
for(outcome in outcomes){
  patients_complications <- complications %>%
    .[`Category of the complication` == outcome, `Code of the patient`]

  data_ <- ate_data %>%
    copy %>%
    .[, c(outcome):='No'] %>%
    .[`Code of the patient` %in% patients_complications, c(outcome):='Yes']
