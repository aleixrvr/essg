...
res <- data.table(variable=coefs_names, coef_value=coefs %>% round(3)) %>% 
  setorder(-coef_value)

variables <- res[, variable]
res[, variable := factor(variable, levels=variables)]

ggplot(res, aes(variable, coef_value)) +
  geom_col() +
