...
![cluters Eurospine 2019](clustering.png)

```{r echo=TRUE}
data_[`Prior Spine Surgery` == 'No' &
  Age <= 41, cluster := 1]
data_[`Prior Spine Surgery` == 'No' &
  Age > 41 &
  `Coronal Balance (C7PL to CSVL)` <= 22.6, 
  cluster := 2]
data_[`Prior Spine Surgery` == 'No' &
  Age > 41 &
  `Coronal Balance (C7PL to CSVL)` > 22.6, 
  cluster := 3]
data_[`Prior Spine Surgery` == 'Yes' & 
    Age <= 59, cluster := 4]
data_[`Prior Spine Surgery` == 'Yes' &
    Age > 59, cluster := 5]
```
...
Number of patients
```{r include=TRUE}
data_[, cluster := as.factor(cluster)]
data_ <- data_[!is.na(cluster)]
...
for(var_ in analysis_vars$radiology ){
  ...
  for(var_y in vars_years){
    ...
    res_treats[['maes']] <- calculate_mae(data_, var_y)
...
for(var_ in analysis_vars$surgery  ){
  ...
  res_treats[['maes']] <- calculate_mae(data_, var_)
