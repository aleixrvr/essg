...
for(var_ in analysis_vars$radiology ){
  ...
  val_5y <- data_[[var_5y]]
  cat('\nPreop vs 6w p-value\n')
  inc <- na.omit(val_6w - val_)
  cat(t.test(inc)$p.val)
  cat('\n\n')
  cat('Preop vs 2y p-value\n')
  inc <- na.omit(val_2y - val_)
  cat(t.test(inc)$p.val)
  cat('\n\n')
  cat('Preop vs 5y p-value\n')
  inc <- na.omit(val_5y - val_)
  cat(t.test(inc)$p.val)
  cat('\n\n')
  cat('6w vs 2y p-value\n')
  inc <- na.omit(val_2y - val_6w)
  cat(t.test(inc)$p.val)
  cat('\n\n')
  cat('2y vs 5y p-value\n')
  inc <- na.omit(val_5y - val_2y)
  cat(t.test(inc)$p.val)
  cat('\n\n')
}
```
