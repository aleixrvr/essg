...
- Upper Thoracic (UT): T2-T6-Pelvis
- Lower Thoracic (LT): T7-T12- Iliac
```{r include=TRUE}
data_[, `Posterior Instrumented Fusion: Upper / Lower Levels`] %>% 
  sapply(. %>% strsplit('-') %>% .[[1]] %>% .[1]) ->
  post_intr_fusion
...
P-valor between UT vs LT 
```{r include=TRUE}
p_val_vars <- c("Prior Spine Surgery", "3CO", "Decompression", "Interbody Fusion")
for(var_ in p_val_vars){
  data_[, .(
    treatment=get(treatment_), variable=get(var_))] ->
    data__
  
  res <- data__[, .(yes = sum(variable == 'Yes'), .N), treatment] %>% 
    .[!is.na(treatment)]
  ...
  p_val <- prop.test(
    c(
      res[treatment==treatment_vals[1]]$yes,
      res[treatment==treatment_vals[2]]$yes
    ),
    c(
      res[treatment==treatment_vals[1]]$N,
      res[treatment==treatment_vals[2]]$N
    ))$p.val
  print("P-value: {p_val}" %>% f)
}
```
