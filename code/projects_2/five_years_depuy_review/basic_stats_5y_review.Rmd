...
```{r, warning=FALSE}
for(name in colnames(clinical_data_0)){
  setnames(clinical_data_0, name, gsub("  ", " ", name, fixed=TRUE))
}
setnames(clinical_data_0, "6Y. Static Major Curve Cobb Angle", "6Y. Static Major curve Cobb angle")
```
...
```{r, warning=TRUE}
years_rad <- sapply(c("6W.", "2Y.", "3Y.", "5Y.", "6Y."), function(x) paste(x, vars$radiology), simplify = TRUE)
years_quality <- sapply(c("6M.", "2Y.", "3Y.", "5Y.", "6Y."), function(x) paste(x, vars$quality), simplify = TRUE)
...
for(var_ in vars$radiology ){
  ...
  dt[is.na(get(var_2y)), c(var_2y) := get(var_3y)]
  dt[is.na(get(var_5y)), c(var_5y) := get(var_6y)]
...
for(var_0 in vars$quality ){
  ...
  print(plot_time(dt, vars_years, middle_point = 0.5))
}
```
