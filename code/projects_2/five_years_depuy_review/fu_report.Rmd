...
xls_path <- '../../data/ESSG Registry FU template.xlsx'
...
clinical_data %<>%
  .[`Site Name` != 'ANK Op'] %>%
  .[`Statut vital` == 'Alive'] %>%
  .[`st1. Date of Stage` %>% as.Date() < as.Date('2020-12-01')] %>%
...
clinical_data %<>%
  .[, radio_2y :=
      !is.na(`2Y. Radiographic parameters - Date`) |
      !is.na(`3Y. Radiographic parameters - Date`)] %>%
  .[, radio_5y :=
      !is.na(`5Y. Radiographic parameters - Date`) |
      !is.na(`6Y. Radiographic parameters - Date`)]
total <- clinical_data[, .N]
```

Proportion of coincidence between calculated followup_2y and "Control > 2y" variable
```{r}
mean(clinical_data[, followup_2y] == clinical_data[, `Control > 2y`])
```

Proportion of coincidence between calculated followup_5y and "Control > 5y" variable
```{r}
mean(clinical_data[, followup_5y] == clinical_data[, `Control > 5y`])
```
...
```{r include=FALSE}
write(clinical_data[followup_2y & !radio_2y, `Code of the patient`], "five_years_depuy_review/patients_2YFU_no2YRadio.txt")
```

```{r include=FALSE}
write(clinical_data[(!followup_2y), `Code of the patient`], "five_years_depuy_review/patients_eligible_no2YFU.txt")
```
...
clinical_data %<>%
  .[`Site Name` != 'ANK Op'] %>%
  .[`Statut vital` == 'Alive'] %>%
  .[`st1. Date of Stage` %>% as.Date() < as.Date('2017-12-01')] %>%
  .[!(`Code of the patient` %in% discarded_patients)] 
  # .[Study=='Op']
```
...
```{r include=FALSE}
write(clinical_data[(!followup_5y), `Code of the patient`], "five_years_depuy_review/patients_eligible_no5YFU.txt")
```

```{r include=FALSE}
write(clinical_data[followup_5y & !radio_5y, `Code of the patient`], "five_years_depuy_review/patients_5YFU_no5YRadio.txt")
```
