...
```{r echo = FALSE}
data_reg_ <- data_reg %>% na.omit
z_cols <- setdiff(colnames(data_reg), c(
  "outcome_gain", "Not resolved or with sequelae"
  )
)
y_var <- data_reg_[, outcome_gain] 
x_var <- data_reg_[, .(`Not resolved or with sequelae`)] 
z_var <- data_reg_[, .SD, .SDcols = z_cols] %>% data.matrix
nonparam_non <- np.reg.test(x_var, y_var, z_var)
```

```{r echo = FALSE}
data_reg_ <- data_reg %>% na.omit
z_cols <- setdiff(colnames(data_reg), c(
  "outcome_gain", "Resolved without sequelae"
  )
)
y_var <- data_reg_[, outcome_gain] 
x_var <- data_reg_[, .(`Resolved without sequelae`)] 
z_var <- data_reg_[, .SD, .SDcols = z_cols] %>% data.matrix
nonparam_res <- np.reg.test(x_var, y_var, z_var)
```
...
res['np_p-value'] <- c(nonparam_non$p.value, nonparam_res$p.value)
res
```
