...
propensity_data <- best_model$clean_data[, .(patient_id, Propensity)] %>% 
  na.omit()
alif_data_propensity <- merge(alif_data, propensity_data, 
  by.x='Code of the patient', by.y = 'patient_id', all=FALSE)

fwrite(alif_data_propensity, 'data/alif_data_propensity.csv')
alif_data <- alif_data_propensity[Propensity<trim_propensity]
```

Patients with 2/3Y FU and propensity less than `r trim_propensity`: `r alif_data[, .N]`
